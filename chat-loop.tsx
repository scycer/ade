...
// Create a custom MCP server with inline tool
...
const createLoggingHooks = () => {
  const hookLogger = async (hookName: string) => {
    return async (input: any, toolUseId: string | undefined) => {
      ...
      if (hookName === 'PreToolUse') {
        ...
        if (input.tool_name === 'Bash' && input.tool_input?.command?.includes('rm')) {
          ...
          // In a real app, you might want to prompt user here
        }
      }
  ...
  return (
    <Box flexDirection="column" height="100%">
      <Box borderStyle="round" borderColor="cyan" flexDirection="column" flexGrow={1} padding={1}>
        ...
        <Text dimColor>Session: {sessionId || 'Not initialized'}</Text>
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Try: "What time is it?" or "Count files in the current directory" or "Show me the package.json"
        </Text>
      </Box>
    </Box>
  );
